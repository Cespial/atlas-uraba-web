<template>
  <header class="fixed top-0 left-0 right-0 z-30 flex items-center justify-between
                 px-4 h-[52px] bg-white border-b border-tensor-border">

    <!-- Logo -->
    <TensorLogo />

    <!-- Centro: selector municipio -->
    <div class="hidden md:flex items-center gap-1">
      <button
        ...
        :class="[
          'px-2.5 py-1 rounded-md font-mono text-[9px] uppercase tracking-[0.12em] transition-all duration-150',
          store.municipioActivo === mun.nombre
            ? 'bg-tensor-teal/15 text-tensor-teal border border-tensor-teal/40'
            : 'text-tensor-muted hover:text-tensor-text hover:bg-black/5 border border-transparent'
        ]"
      >{{ mun.short }}</button>
    </div>
    ...
    <div class="flex items-center gap-2">
      <span class="hidden sm:flex items-center gap-1.5 px-2 py-1 rounded-md
                   bg-tensor-bg border border-tensor-border">
        <span class="w-1.5 h-1.5 rounded-full bg-tensor-teal animate-pulse" />
        <span class="font-mono text-[9px] uppercase tracking-[0.12em] text-tensor-muted">
          7,028 manzanas
        </span>
      </span>
